...
function Parameters(
    ...
    k3 = 6.07e-3,#3  # k3 rate constant for suppression of hepatic glucose release by change of plasma glucose
    k4 = 2.35e-4,#4  # k4 rate constant for suppression of hepatic glucose release by delayed (remote) insulin
    k5 = 0.0424,   # k5 rate constant for delayed insulin dependent uptake of glucose
    k6 = 2.2975,   # k6 rate constant for stimulation of insulin production by the change of plasma glucose concentration (beta cell funtion)
    k7 =  0.4,#1.15,     # k7 rate constant for integral of glucose on insulin production (beta cell function)
    k8 = 5,#7.27,    # k8 rate constant for the simulation of insulin production by the rate of change in plasma glucose concentration (beta cell function)
    k9 = 6.00e-2,#3.83e-2,  # k9 rate constant for outflow of insulin from plasma to interstitial space
    k10 = 2.84e-1, # k10 rate constant for degredation of insulin in remote compartment
